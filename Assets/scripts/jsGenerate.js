...
var ROOM_RELAX : byte = 14;
var ROOM_BAR : byte = 15;
var ROOM_CEO : byte = 16;
var ROOM_MIDDLE_MANAGEMENT : byte = 17;
...
function PlaceRooms(in_roomType:byte, in_connectToFloorType:byte, in_width:int, in_depth:int, in_minCount:int, in_maxCount:int)
{
	...
	for(var iter:int = 0; iter < 5000; iter++)
	{
		...
		if (idealCount > -1)
		{
			...
			{
				if (dir == 0)	wallMapX[startX+doorOffset+i2*sajsWidth, startY+sajsDepth] = 2;
				else if (dir == 1)	wallMapY[startX+sajsDepth, startY+doorOffset+i2*sajsWidth] = 2;
				else if (dir == 2)	wallMapX[startX-doorOffset-i2*sajsWidth, startY-sajsDepth+1] = 2;
				else if (dir == 3)	wallMapY[startX-sajsDepth+1, startY-doorOffset-i2*sajsWidth] = 2;
